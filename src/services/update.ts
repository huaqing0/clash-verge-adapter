...
// Adapter builds replace upstream Clash Verge with an MCP-aware build. Letting
// the upstream updater run would silently replace the adapter and remove its
// local control API. Updates are therefore performed only by the adapter's
// verified installer/upgrade transaction.
export const UPSTREAM_UPDATES_DISABLED = true

export const checkUpdateSafe = async (
  options?: CheckOptions,
): Promise<Update | null> => {
  if (UPSTREAM_UPDATES_DISABLED) return null
