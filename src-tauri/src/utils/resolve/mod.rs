...
async fn init_silent_updater() {
    ...
    // This is an MCP Adapter build. Installing an upstream Clash Verge update
    // would remove the adapter API, so updates must go through the Adapter's
    // verified installer/upgrade transaction instead. Also discard any update
    // downloaded by the official app before this build was installed.
    if SilentUpdater::upstream_updates_disabled_for_build() {
        SilentUpdater::global().disable_for_adapter_build();
        logging!(
            info,
            Type::Setup,
            "Upstream updater disabled for Clash Verge Adapter build"
        );
        return;
    }
