...
fn determine_update_flags(patch: &IVerge) -> UpdateFlags {
    ...
    if patch.hotkeys.is_some() || enable_global_hotkey.is_some() {
        update_flags.insert(UpdateFlags::HOTKEY);
    }
...
async fn process_terminated_flags(update_flags: UpdateFlags, patch: &IVerge) -> Result<()> {
    ...
    if update_flags.contains(UpdateFlags::HOTKEY) {
        ...
        let hotkeys = verge.hotkeys.clone().unwrap_or_default();
        let enable_global_hotkey = verge.enable_global_hotkey.unwrap_or(true);
        let should_register = hotkey::Hotkey::should_register_user_hotkeys(enable_global_hotkey);
        hotkey::Hotkey::global()
            .update_registration_state(hotkeys, should_register)
            .await?;
    }
