...
impl SilentUpdater {
    ...
    /// Read the policy embedded into this exact application build. Unknown or
    /// malformed policies fail closed; only an explicit `upstream` policy can
    /// enable Clash Verge's built-in updater.
    pub fn upstream_updates_disabled_for_build() -> bool {
        const ADAPTER_MANIFEST: &str = include_str!("../../resources/adapter-manifest.json");

        serde_json::from_str::<serde_json::Value>(ADAPTER_MANIFEST)
            .ok()
            .and_then(|manifest| manifest.get("upstreamUpdaterPolicy")?.as_str().map(str::to_owned))
            .as_deref()
            != Some("upstream")
    }

    /// Prevent an upstream Clash Verge package from replacing the MCP Adapter.
    ///
    /// Adapter releases are installed by the external verified transaction,
    /// not by Clash Verge's upstream updater. Clearing a cache created by the
    /// official app also prevents a stale package from being installed later.
    pub fn disable_for_adapter_build(&self) {
        self.update_ready.store(false, Ordering::Release);
        *self.pending_bytes.write() = None;
        *self.pending_update.write() = None;
        *self.pending_version.write() = None;
        Self::delete_cache();
    }
}
...
mod tests {
    ...
    #[test]
    fn adapter_build_disables_upstream_updater() {
        assert!(SilentUpdater::upstream_updates_disabled_for_build());
    }
}
